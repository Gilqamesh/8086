...
static int file_reader__read(struct file_reader *self, int size) {
    if (file_reader__available(self) < size) {
        self->error_handler("in 'file_reader__read': cannot read size much", FILE_READER_ERROR_WARN);
    ...
    if (read_amount == -1) {
        perror("read");
        self->error_handler("in 'file_reader__read'", FILE_READER_ERROR_FATAL);
    }
...
void file_reader__read_byte(struct file_reader* self, void* out) {
    if (file_reader__eof_reached(self)) {
        self->error_handler("in 'file_reader__read_byte': already reached end of file, nothing to read", FILE_READER_ERROR_FATAL);
        return ;
    }
